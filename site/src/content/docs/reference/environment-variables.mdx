...
| `HUB_SUPABASE_SECRET_KEY` | Yes | Dashboard's `sb_secret_` key for collector inventory sync and writes |
...
| Variable | Required | Purpose |
|---|---|---|
| `SNAPSHOT_RETENTION_DAYS` | Optional | Days of snapshot history to keep (default `90`, minimum `31`) |
